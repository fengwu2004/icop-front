    ...
    <template v-for="item in routes" v-if="!item.hidden&&item.children">
      <el-submenu :index="getMenuIndex(item)" :key="item.name">
        <template slot="title">
          ...
          <span v-if="item.meta&&item.meta.title" slot="title" style="color:#16325C">{{ generateTitle(item.meta.title) }}</span>
        </template>
        <template v-for="(child, index) in item.children" v-if="!child.hidden">
          ...
          <router-link v-else :to="composePath(item, child)" :key="child.name">
            <el-menu-item :index="composePath(item, child)">
              ...
              <span v-if="child.meta&&child.meta.title" slot="title" style="color:#16325C">{{ generateTitle(child.meta.title) }}</span>
            </el-menu-item>
          </router-link>
        </template>
      </el-submenu>
